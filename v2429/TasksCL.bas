...
Sub listsButtonFilter_Click
	...
	Filter.GetTaskFiltersDialog("TasksCL", Curr, CurrentTab, sft)
End Sub
...
Sub ShowTaskDetails(tagcode As String)
	...
	If Record.RowCount > 0 Then
		Dim Filter As AppDialogs
		Filter.Initialize
		Filter.getTaskDetailsDialog(Activity, "TasksCL", ShareCode.tarefasavisoInfoDescritiva, Record, tagcode)
	Else
...
'			DBUtils.InsertMaps(Starter.LocalSQLEVC, "dta_requests", ListOfMaps)
			Utils.InsertMapsWithLog(Starter.LocalSQLEVC, "dta_requests", ListOfMaps,newRequest,"")
...
'			DBUtils.InsertMaps(Starter.LocalSQLEVC, "dta_requests_relations", ListOfMaps)
			Utils.InsertMapsWithLog(Starter.LocalSQLEVC, "dta_requests_relations", ListOfMaps,newRequest,"")
					
			
			Dim maxIDi As Int = DBStructures.GetScriptColumnIntEVC("select max(id) as id from dta_requests_values", "id") + 1 + 1000000
			
			'Dim maxID As Int = DBStructures.GetMaxOfEVC("dta_requests_values", "id", "") + 1 + 100000
			...
			If (incolitems.RowCount >= 1) Then
				...
				'Utils.CallApi(params, Me, "https://demo.${ShareCode.APPL_HOST}/api/orion/clarel-satus/update")
			...
			End If
		Else
			MsgboxAsync("Não foi possível verificar o tipo de intervenção/serviço associado à Checklist. Efectue a actualização/sincronização de dados e tente novamente.", "Alerta!")
		End If
	Catch
		Log(LastException)
	End Try
