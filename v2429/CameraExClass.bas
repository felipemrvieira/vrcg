...
#if OPTIMAL
	'HERE!!! Set Optimal preview and picture sizes as calculated before
	If Main.useOptimalCalculatedValues Then
		r.RunMethod3("setPreviewSize",Main.optimalPreviewSize(0),"java.lang.int",Main.optimalPreviewSize(1),"java.lang.int")
		r.RunMethod3("setPictureSize",Main.optimalPictureSize(0),"java.lang.int",Main.optimalPictureSize(1),"java.lang.int")
	End If 	
#End if	
	Sleep(100)
	CommitParameters
...
Private Sub Camera_Ready (Success As Boolean)
	...
	Else
		Log("success = false, " & LastException.Message)
	End If
...
Public Sub GetSupportedPreviewSizes As CameraSize()
	...
	For i = 0 To list1.Size - 1
		...
		'cs(i).Initialize(r.GetField("width"), r.GetField("height"))
		cs(i).Initialize
...
'	cs.Initialize(r.GetField("width"), r.GetField("height"))
	cs.Initialize
