...
#DesignerProperty: Key: UpDownIcon, DisplayName: UpDown Icon, FieldType: Boolean, DefaultValue: False, Description: Whether UP & DOWN icon exists
#DesignerProperty: Key: UpDownIndex, DisplayName: UpDown Icon Index, FieldType: Int, DefaultValue: 0, Description: Identify the index of UP & DOWN icon image
Sub Class_Globals
	Private mBase As B4XView
	Private mBaseParent As B4XView
	Private msvParent As B4XView
	Public sv As B4XView
	Type CLVExpItem(Panel As B4XView, Size As Int, Value As Object, Color As Int, TextItem As Boolean, Offset As Int)
	...
	Private mUpDownIconExists As Boolean = False
	Private mUpDownIconIndex As Int = 0
	Private iData As List
#if B4J
	Private fx As JFX
#else if B4A
...
Public Sub DesignerCreateView (Base As Object, Lbl As Label, Props As Map)
	...
	mUpDownIconExists = Props.GetDefault("UpDownIcon", False)
	mUpDownIconIndex = Props.GetDefault("UpDownIndex", 0)
...
Sub AnimatedArrow(index As Int, From As Int, ToDegree As Int)
	Dim InnerReference As ExpandedListView = Me
	Dim pnlTitle As B4XView = InnerReference.GetPanel(index).GetView(0) 		'pnlTitle is the first item
	Dim iv As B4XView = pnlTitle.GetView(mUpDownIconIndex) 						'ImageView1 is the second item
	iv.SetRotationAnimated(0, From)
	iv.SetRotationAnimated(InnerReference.AnimationDuration, ToDegree)
End Sub

Sub CollapseItem(index As Int)
	Dim InnerReference As ExpandedListView = Me
	Dim id As ItemData = InnerReference.GetValue(index)
	InnerReference.ResizeItem(index, id.CollapsedHeight)
	InnerReference.GetPanel(index).Tag = False
	If (mUpDownIconExists) Then
...
Public Sub Base_Resize (Width As Double, Height As Double)
	...
	If horizontal Then
		...
		For Each it As CLVExpItem In items
...
Public Sub getUpDownIconExists As Boolean
	Return mUpDownIconExists
End Sub

Public Sub setUpDownIconExists(val As Boolean)
	mUpDownIconExists = val
End Sub

Public Sub getUpDownIconIndex As Int
	Return mUpDownIconIndex
End Sub

Public Sub setUpDownIconIndex(val As Int)
	mUpDownIconIndex = val
End Sub


Public Sub getScrollHeight As Int
	Return sv.ScrollViewContentHeight
End Sub

Public Sub getItemsHeight As Int
	Dim ret As Int = 0
	For Each it As CLVExpItem In items
		ret = ret + it.Panel.Height
	Next
	Return ret
End Sub
...
'Returns the CLVExpItem. Should not be used in most cases.
Public Sub GetRawListItem(Index As Int) As CLVExpItem
	Return items.Get(Index)
End Sub

Public Sub GetItemData(Index As Int) As Object
...
Public Sub GetPanel(Index As Int) As B4XView
	...
	If items.Size > Index Then
		Return GetRawListItem(Index).Panel.GetView(0)
	Else
		Return GetRawListItem(0).Panel.GetView(0)
	End If
End Sub
...
Public Sub RemoveAt(Index As Int)
	If getSize <= 1 Then
		...
		Return
	End If
	Dim RemoveItem As CLVExpItem = items.Get(Index)
	Dim p As B4XView
	For i = Index + 1 To items.Size - 1
		Dim item As CLVExpItem = items.Get(i)
		p = item.Panel
		p.Tag = i - 1
...
Public Sub AddTextItemWithData(Text As Object, Value As Object, Data As Object)
	InsertAtTextItem(items.Size, Text, Value, Data)
End Sub

'Inserts a text item at the specified index.
Public Sub InsertAtTextItem(Index As Int, Text As Object, Value As Object, Data As Object)
	If horizontal Then
	...
	InsertAtWithData(Index, pnl, Value, Data)
	Dim item As CLVExpItem = GetRawListItem(Index)
	item.TextItem = True
End Sub
...
Public Sub InsertAtWithData(Index As Int, pnl As B4XView, Value As Object, Data As Object)
	Dim size As Float
	If horizontal Then
		size = pnl.Width
	Else
		size = pnl.Height
	End If
	InsertAtImpl(Index, pnl, size, Value, 0, Data)
End Sub

	
Private Sub InsertAtImpl(Index As Int, Pnl As B4XView, ItemSize As Int, Value As Object, InitialSize As Int, Data As Object)
	...
	Else
		...
		If Index = 0 Then
			offset = mDividerSize
		Else
			Dim PrevItem As CLVExpItem = items.Get(Index - 1)
			offset = PrevItem.Offset + PrevItem.Size + mDividerSize
		End If
		For i = Index To items.Size - 1
			Dim It As CLVExpItem = items.Get(i)
...
Private Sub SetItemOffset (item As CLVExpItem, offset As Float)
	item.Offset = offset
	If horizontal Then
...
Public Sub ReplaceAt(Index As Int, pnl As B4XView, ItemHeight As Int, Value As Object)
	Dim RemoveItem As CLVExpItem = items.Get(Index)
	items.RemoveAt(Index)
	Dim oData As Object = iData.Get(Index)
	iData.RemoveAt(Index)
	RemoveItem.Panel.RemoveViewFromParent
	InsertAtImpl(Index, pnl, ItemHeight, Value, RemoveItem.Size, oData)
End Sub

Public Sub ReplaceAtWithData(Index As Int, pnl As B4XView, ItemHeight As Int, Value As Object, Data As Object)
	Dim RemoveItem As CLVExpItem = items.Get(Index)
	items.RemoveAt(Index)
	iData.RemoveAt(Index)
	RemoveItem.Panel.RemoveViewFromParent
	InsertAtImpl(Index, pnl, ItemHeight, Value, RemoveItem.Size, Data)
End Sub
...
Sub AddWithData(Pnl As B4XView, Value As Object, Data As Object)
	InsertAtWithData(items.Size, Pnl, Value, Data)
End Sub
...
Public Sub FindIndexFromOffset(Offset As Int) As Int
	...
	Do While True
		Dim CurrentItem As CLVExpItem = items.Get(Position)
...
Public Sub getBaseParent As B4XView
	Return mBaseParent
End Sub

Public Sub getSVParent As B4XView
	Return msvParent
End Sub
