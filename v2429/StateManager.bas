...
Private Sub innerSaveState(v As View, list1 As List)
	...
	If v Is EditText Then
		...
		edit = v
		data = Array As Object(edit.Text, edit.SelectionStart, edit.Tag, edit.Enabled, edit.Visible)
	Else If v Is Spinner Then
		Dim spinner1 As Spinner
		spinner1 = v
		data = Array As Object(spinner1.SelectedIndex, spinner1.Tag, spinner1.Enabled, spinner1.Visible)
	Else If v Is CheckBox Then
		Dim check As CheckBox
		check = v
		data = Array As Object(check.Checked, check.Tag, check.Enabled, check.Visible)
	Else If v Is RadioButton Then
		Dim radio As RadioButton
		radio = v
		data = Array As Object(radio.Checked, radio.Tag, radio.Enabled, radio.Visible)
	Else If v Is ToggleButton Then
		Dim toggle As ToggleButton
		toggle = v
		data = Array As Object(toggle.Checked, toggle.Tag, toggle.Enabled, toggle.Visible)
	Else If v Is SeekBar Then
		Dim seek As SeekBar
		seek = v
		data = Array As Object(seek.Value, seek.Tag, seek.Enabled, seek.Visible)
	Else If v Is TabHost Then
		Dim th As TabHost
		th = v
		data = Array As Object(th.CurrentTab, th.Tag, th.Enabled, th.Visible)
	...
	Else If v Is Panel Then
		Dim panel1 As Panel
		panel1 = v
		data = Array As Object(panel1.Tag, panel1.Enabled, panel1.Visible)
...
Private Sub innerRestoreState(v As View, list1 As List)
	...
	Else If v Is Spinner Then
		...
		spinner1.Tag = data(1)
		spinner1.Enabled = data(2)
		spinner1.Visible = data(3)
	Else If v Is CheckBox Then
		Dim check As CheckBox
		check = v
		data = getNextItem(list1)
		...
		check.Tag = data(1)
		check.Enabled = data(2)
		check.Visible = data(3)
	Else If v Is RadioButton Then
		Dim radio As RadioButton
		radio = v
		data = getNextItem(list1)
		...
		radio.Tag = data(1)
		radio.Enabled = data(2)
		radio.Visible = data(3)
	Else If v Is ToggleButton Then
		Dim toggle As ToggleButton
		toggle = v
		data = getNextItem(list1)
		...
		toggle.Tag = data(1)
		toggle.Enabled = data(2)
		toggle.Visible = data(3)
	Else If v Is SeekBar Then
		Dim seek As SeekBar
		seek = v
		data = getNextItem(list1)
	...
	Else If v Is Panel Then
		Dim panel1 As Panel
		panel1.Tag = data(0)
		panel1.Enabled = data(1)
		panel1.Visible = data(2)
		panel1 = v
		For i = 0 To panel1.NumberOfViews - 1
